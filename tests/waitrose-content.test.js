...
const SCRIPT_PATH = path.join(__dirname, "..", "dist", "waitrose-content.js");
...
function createSortContainer(doc) {
  ...
  button.append(span);
  button.append(doc.createElementNS("http://www.w3.org/2000/svg", "svg"));
...
function createProductGrid(doc, products) {
  ...
  for (const product of products) {
    ...
    if (product.unitPrice) {
      ...
      priceSpan.append(srOnly);

      priceSpan.append(doc.createTextNode(product.unitPrice));
      article.append(priceSpan);
    }

    if (product.name) {
      const nameEl = doc.createElement("span");
      nameEl.textContent = product.name;
      article.append(nameEl);
    }
...
test("findSortContainers finds all sort containers", (t) => {
  ...
  env.document.body.append(sort1.container);
  env.document.body.append(sort2.container);
...
test("injectValueOption adds option when dropdown opens", async (t) => {
  ...
  env.document.body.append(sort.container);
  ...
  sort.contentWrapper.append(options);
...
test("sortByUnitPrice sorts products ascending", (t) => {
  ...
  env.document.body.append(grid);

  env.hooks.sortByUnitPrice();

  const cards = grid.querySelectorAll('[class*="productPod___"]');
  const names = [...cards].map((c) => c.querySelector("span:last-child").textContent);
...
test("observeProductGrid re-sorts when new products are added", async (t) => {
  ...
  srOnly.textContent = "Price per unit";
  priceSpan.append(srOnly);
  priceSpan.append(env.document.createTextNode("\u00A31.00/kg"));
  newArticle.append(priceSpan);
  const nameSpan = env.document.createElement("span");
  nameSpan.textContent = "New cheap";
  newArticle.append(nameSpan);
...
test("extractUnitPrice handles pence format correctly", (t) => {
  ...
  srOnly.textContent = "Price per unit";
  priceSpan.append(srOnly);
  priceSpan.append(env.document.createTextNode("46.7p each"));
  article.append(priceSpan);
